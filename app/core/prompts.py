# Промпт для генерации идей (стандартный, 3-5 идей)
...
# Новый промпт для пакетной генерации большого количества идей
IDEA_GENERATION_BATCH_PROMPT = '''Ты — эксперт по научным исследованиям и инновациям.
Сгенерируй {n_ideas} уникальных и разнообразных гипотез для научного проекта.
Каждая идея должна обладать научной новизной и быть реализуемой с учетом компетенций пользователя.
Идеи должны отличаться друг от друга, охватывать разные аспекты указанной темы.
...
Ответ строго в формате JSON с ключом "ideas", содержащим массив из {n_ideas} объектов.
Каждый объект имеет поля: "title", "description", "novelty_aspect", "feasibility_assessment".'''

# Новый промпт для анализа широты темы
TOPIC_ANALYSIS_PROMPT = '''Ты — эксперт по оценке научных направлений.
Проанализируй запрос пользователя и определи, сколько различных научных гипотез можно сгенерировать на эту тему.
Учитывай:
- Широту предметной области (например, "применение ИИ в медицине" даст много идей, а "оптимизация конкретного катализатора" — мало).
- Наличие подтем и ответвлений.
- Степень детализации, указанную пользователем.

Ответ должен быть в формате JSON с полем "recommended_ideas" (целое число от 5 до 250).
